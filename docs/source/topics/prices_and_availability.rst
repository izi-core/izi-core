...
This page explains how prices and availability are determined in IZI.  In
...
IZI, on the other hand, distinguishes products from stockrecords and provides
a swappable 'strategy' component for selecting the appropriate stockrecord,
calculating prices and availability information.

.. code-block:: python

   >>> from izi.apps.partner.strategy import Selector
   >>> product = Product.objects.get(id=1)
...
IZI uses a 'strategy' object to determine product availability and pricing.  A
new strategy instance is assigned to the request by the basket middleware.  A
:class:`~izi.apps.partner.strategy.Selector`
...
The strategy class is accessed in several places in IZI's codebase.  In templates, a
...
.. autoclass:: izi.apps.partner.strategy.Base
   :members: fetch_for_product, fetch_for_parent, fetch_for_line
   :noindex:

IZI also provides a "structured" strategy class which provides overridable
methods for selecting the stockrecord, and determining pricing and availability
policies:

.. autoclass:: izi.apps.partner.strategy.Structured
   :members:
   :noindex:

For most projects, subclassing and overriding the ``Structured`` base class
should be sufficient.  However, IZI also provides mixins to easily compose the
appropriate strategy class for your domain.

Currency
--------

IZI allows you to define a currency code for each stock - a text field that
defaults to ``settings.IZI_DEFAULT_CURRENCY``.

By default, IZI expects all products added to a single basket to have the
...
:func:`~izi.apps.partner.strategy.Structured.select_stockrecord` method.
IZI does not determine or store user currency and uses it only for
...
.. autoclass:: izi.apps.partner.strategy.Selector
   :members:
   :noindex:
...
.. autoclass:: izi.apps.partner.prices.Base
   :members:
   :noindex:

There are also several policies that accommodate common scenarios:

.. automodule:: izi.apps.partner.prices
...
.. autoclass:: izi.apps.partner.availability.Base
   :members:
   :noindex:

There are also several pre-defined availability policies:

.. automodule:: izi.apps.partner.availability
...
IZI also ships with several mixins which implement one method of the
``Structured`` strategy.  These allow strategies to be easily
composed from re-usable parts:

.. automodule:: izi.apps.partner.strategy
...
IZI's default ``Selector`` class returns a ``Default`` strategy built from
the strategy mixins:
...
- Always picks the first stockrecord (this is backwards compatible with
  IZI<0.6 where a product could only have one stockrecord).
- Charge no tax.
...
.. code-block:: python
    ...
    from izi.apps.partner import strategy, prices
