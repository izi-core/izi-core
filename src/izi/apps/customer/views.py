...
from izi.core.utils import safe_referrer
from izi.views.generic import PostActionMixin
...
class AccountSummaryView(generic.RedirectView):
    ...
    IZI defaults to just redirecting to the profile summary page (and
    that redirect can be configured via IZI_ACCOUNT_REDIRECT_URL), but
    ...
    """
    pattern_name = settings.IZI_ACCOUNTS_REDIRECT_URL
    permanent = False
...
class LogoutView(generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for cookie in settings.IZI_COOKIES_DELETE_ON_LOGOUT:
...
class ProfileDeleteView(PageTitleMixin, generic.FormView):
    ...
    active_tab = 'profile'
    success_url = settings.IZI_HOMEPAGE
...
class EmailHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.IZI_EMAILS_PER_PAGE
...
class OrderHistoryView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.IZI_ORDERS_PER_PAGE
    model = Order
...
class AddressListView(PageTitleMixin, generic.ListView):
    ...
    paginate_by = settings.IZI_ADDRESSES_PER_PAGE
