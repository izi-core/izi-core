...
def metadata(request):
    ...
    """
    return {'shop_name': settings.IZI_SHOP_NAME,
            'shop_tagline': settings.IZI_SHOP_TAGLINE,
            'homepage_url': settings.IZI_HOMEPAGE,
            'language_neutral_url_path': strip_language_code(request),
            # Fallback to old settings name for backwards compatibility
            'use_less': (getattr(settings, 'IZI_USE_LESS', None) or
                         getattr(settings, 'USE_LESS', False)),
            'google_analytics_id': (getattr(settings, 'IZI_GOOGLE_ANALYTICS_ID', None) or
