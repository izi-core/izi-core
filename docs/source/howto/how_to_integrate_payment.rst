...
IZI is designed to be very flexible around payment.  It supports paying for an
...
* ``SourceType`` - This is the type of payment source used (eg PayPal, DataCash).  As part of setting up
  a new IZI site you would create a SourceType for each of the payment
...
By default, IZI's checkout does not provide any payment integration as it is
...
successful, then nothing needs to be returned.  However, IZI defines a few
common exceptions which can occur:

* ``izi.apps.payment.exceptions.RedirectRequired``  For payment integrations
  that require redirecting the user to a 3rd-party site.  This exception class
  has a ``url`` attribute that needs to be set.

* ``izi.apps.payment.exceptions.UnableToTakePayment`` For *anticipated* payment
  problems such as invalid bankcard number, not enough funds in account - that kind
  of thing.

* ``izi.apps.payment.exceptions.UserCancelled`` During many payment flows,
...
* ``izi.apps.payment.exceptions.PaymentError``  For *unanticipated* payment
...
* Create the appropriate ``izi.apps.payment.models.Source`` instance and pass
...
For example::
    ...
    from izi.apps.payment import models


    # Subclass the core IZI view so we can customise
