...
sandbox_load_data:
	...
	sandbox/manage.py izi_import_catalogue sandbox/fixtures/*.csv
	sandbox/manage.py izi_import_catalogue_images sandbox/fixtures/images.tar.gz
	sandbox/manage.py izi_populate_countries --initial-only
...
sandbox_image:
	docker build -t izi-core-sandbox:latest .
...
coverage: venv
	$(PYTEST) --cov=izi --cov-report=term-missing
...
messages:
	...
	cd src/izi; django-admin.py makemessages -a

compiledmessages:
	# Compile the gettext files
	cd src/izi; django-admin.py compilemessages
...
todo:
	...
	# IZI dropping support for a Django version)
	-grep -rnH TODO *.txt
	-grep -rnH TODO src/izi/apps/
	-grep -rnH "django.VERSION" src/izi/apps
