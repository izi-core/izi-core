...
IZI provides only a skeleton for building your order processing pipeline on
...
There are two relevant IZI apps to order processing.
...
IZI models order processsing through events.  There are three types to be
aware of:
...
Most IZI shops will want to customise the ``EventHandler`` class from the
order app.  This class is intended to handle all events and perform the
appropriate actions.  The main public API is

.. autoclass:: izi.apps.order.processing.EventHandler
...
It can be useful to use order and line statuses too.  IZI provides some helper
methods to make this easier.

.. autoclass:: izi.apps.order.abstract_models.AbstractOrder
   :members: pipeline, all_statuses, available_statuses, set_status
   :noindex:

.. autoclass:: izi.apps.order.abstract_models.AbstractLine
   :members: pipeline, all_statuses, available_statuses, set_status
   :noindex:
...
the functionality described here is not in IZI.  However, IZI provides the
