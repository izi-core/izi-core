...
from izi.test.factories.utils import tax_add, tax_subtract
...
class OrderFactory(factory.DjangoModelFactory):
    ...
    if hasattr(settings, 'IZI_INITIAL_ORDER_STATUS'):
        status = settings.IZI_INITIAL_ORDER_STATUS

    site_id = settings.SITE_ID
    number = factory.LazyAttribute(lambda o: '%d' % (100000 + o.basket.pk))
    basket = factory.SubFactory(
        'izi.test.factories.BasketFactory')
...
class OrderLineFactory(factory.DjangoModelFactory):
    ...
    product = factory.SubFactory(
        'izi.test.factories.ProductFactory')
