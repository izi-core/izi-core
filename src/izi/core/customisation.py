...
def inherit_app_config(local_app_path, app_package, app_label):
    ...
    create_file(
        ...
        "from izi.apps.{app_label} import config\n\n\n"
...
def fork_app(label, folder_path, logger=None):
    """
    Create a custom version of one of IZI's apps
    ...
    valid_labels = [x.replace('izi.apps.', '') for x in izi.IZI_CORE_APPS
                    if x.startswith('izi')]
    if label not in valid_labels:
        raise ValueError("There is no IZI app that matches '%s'" % label)
    ...
    izi_app_path = join(izi.__path__[0], 'apps', label_folder)
    if exists(os.path.join(izi_app_path, 'admin.py')):
        logger.info("Creating admin.py")
        create_file(join(local_app_path, 'admin.py'),
                    "from izi.apps.%s.admin import *  # noqa\n" % label)

    logger.info("Creating app config")
    inherit_app_config(local_app_path, app_package, label)

    # Only create models.py and migrations if it exists in the IZI app
    izi_models_path = join(izi_app_path, 'models.py')
    if exists(izi_models_path):
        logger.info("Creating models.py")
        create_file(
            join(local_app_path, 'models.py'),
            "from izi.apps.%s.models import *  # noqa isort:skip\n" % label)

        migrations_path = 'migrations'
        source = join(izi_app_path, migrations_path)
        if exists(source):
    ...
    msg = (
        ...
        "(replacing the equivalent IZI app). This can be "
        "achieved using IZI's get_core_apps function - e.g.:"
    ) % app_package
    snippet = (
        "  # settings.py\n"
        "  ...\n"
