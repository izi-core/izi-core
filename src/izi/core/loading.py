...
from izi.core.exceptions import (
    AppNotFoundError, ClassNotFoundError, ModuleNotFoundError)


def get_class(module_label, classname, module_prefix='izi.apps'):
    """
...
def get_class_loader():
    return import_string(settings.IZI_DYNAMIC_CLASS_LOADER)


def get_classes(module_label, classnames, module_prefix='izi.apps'):
...
def default_class_loader(module_label, classnames, module_prefix):
    ...
    Examples:
        ...
        izi.apps.dashboard.catalogue.forms.ProductForm
        ...
        [izi.apps.dashboard.catalogue.forms.ProductForm,
         izi.apps.dashboard.catalogue.forms.StockRecordForm]
    ...
    # import from IZI package (should succeed in most cases)
    # e.g. 'izi.apps.dashboard.catalogue.forms'
    izi_module_label = "%s.%s" % (module_prefix, module_label)
    izi_module = _import_module(izi_module_label, classnames)

    # returns e.g. 'izi.apps.dashboard.catalogue',
    ...
    if installed_apps_entry.startswith('%s.' % module_prefix):
        # The entry is obviously an IZI one, we don't import again
        local_module = None
    else:
    ...
    if izi_module is local_module is None:
    ...
    return _pluck_classes([local_module, izi_module], classnames)
...
def get_profile_class():
    ...
    # makes sense for IZI to continue to use it. Projects built on Django
...
def feature_hidden(feature_name):
    """
    Test if a certain IZI feature is disabled.
    """
    return (feature_name is not None and
            feature_name in settings.IZI_HIDDEN_FEATURES)
...
def is_model_registered(app_label, model_name):
    ...
    register IZI models if they aren't overridden by a forked app.
    """
    try:
