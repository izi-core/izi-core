...
IZI comes with a pre-configured dashboard navigation that gives you access
...
``IZI_DASHBOARD_NAVIGATION`` setting in you settings file.
...
``access_fn`` parameter for the current node, so that IZI is able to
...
IZI uses `Font Awesome`_ for its icons which makes it very simple to add
...
If no ``'access_fn'`` is specified, ``IZI_DASHBOARD_DEFAULT_ACCESS_FUNCTION``
is used.