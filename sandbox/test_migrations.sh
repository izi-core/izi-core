...
then
  ...
  mysql -u root -e "DROP DATABASE IF EXISTS izi_travis; CREATE DATABASE izi_travis"
  mysql -u root -e "GRANT ALL PRIVILEGES ON izi_travis.* TO 'travis'@'localhost' IDENTIFIED BY '';"

  echo "Creating Postgres database and user"
  psql -c "DROP ROLE IF EXISTS travis"
  psql -c "CREATE ROLE travis LOGIN PASSWORD ''"
  psql -c "DROP DATABASE IF EXISTS izi_travis"
  psql -c "CREATE DATABASE izi_travis"
fi
