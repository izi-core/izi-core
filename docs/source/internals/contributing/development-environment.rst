...
Fork the repo and run::
    ...
    $ git clone git@github.com:<username>/izi-core.git
    $ cd izi-core
...
As the sandbox is a vanilla IZI site, it is what we use to build migrations
against::
...
IZI's CSS files are built using LESS_.  However, the sandbox defaults to
...
2. Run helper script::
    ...
    This will recreate the IZI database in both MySQL and Postgres and rebuild
    it using ``migrate``.
