...
from izi.apps.catalogue.search_handlers import (
...
class TestProductSearchHandlerSetting(TestCase):
    def test_product_search_handler_setting(self):
        """
        Test that the `IZI_PRODUCT_SEARCH_HANDLER` setting, when set,
        ...
        with override_settings(IZI_PRODUCT_SEARCH_HANDLER=handler_override):
