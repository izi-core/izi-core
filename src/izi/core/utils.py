...
def default_slugifier(value, allow_unicode=False):
    """
    IZI's default slugifier function. When unicode is allowed
...
def slugify(value):
    ...
    The IZI_SLUG_FUNCTION can be set with a dotted path to the slug
    function to use, defaults to 'izi.core.utils.default_slugifier'.

    IZI_SLUG_MAP can be set of a dictionary of target:replacement pairs

    IZI_SLUG_BLACKLIST can be set to a iterable of words to remove after
    ...
    for k, v in settings.IZI_SLUG_MAP.items():
        value = value.replace(k, v)

    slugifier = import_string(settings.IZI_SLUG_FUNCTION)
    slug = slugifier(value, allow_unicode=settings.IZI_SLUG_ALLOW_UNICODE)

    # Remove stopwords from slug
    for word in settings.IZI_SLUG_BLACKLIST:
...
def format_datetime(dt, format=None):
    ...
    if is_naive(dt):
        ...
        logging.warning(
            "izi.core.utils.format_datetime received native datetime")
    else:
...
def get_default_currency():
    ...
    IZI_DEFAULT_CURRENCY as something it needs to generate a migration for.
    """
    return settings.IZI_DEFAULT_CURRENCY
