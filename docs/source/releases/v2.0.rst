...
IZI 2.0 is compatible with Django 1.11, Django 2.0 and Django 2.1 as well as
Python 3.5 and 3.6.
...
- Backward-compatible dynamic loading of formset classes that have moved in IZI was
...
- We've dropped the dependency on Unidecode due to license incompatibilities,
  ``izi.core.utils.default_slugifier`` now uses ``izi.core.utils.cautious_slugify``
  to handle unicode characters in slugs when ``IZI_SLUG_ALLOW_UNICODE`` is ``False``.

- ``IZI_SLUG_FUNCTION`` previously accepted a function as its value. It now
...
Backwards incompatible changes in IZI 2.0
-------------------------------------------
...
- Redirection to the parent detail view for child products is disabled by default.
  ...
  ``izi.apps.catalogue.views.ProductDetailView.enforce_parent`` to ``False``.
