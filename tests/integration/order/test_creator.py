...
from izi.apps.catalogue.models import ProductClass, Product
from izi.apps.checkout import calculators
from izi.apps.offer.utils import Applicator
from izi.apps.order.models import Order
from izi.apps.order.utils import OrderCreator
from izi.apps.shipping.methods import Free, FixedPrice
from izi.apps.shipping.repository import Repository
from izi.apps.voucher.models import Voucher
...
class TestSuccessfulOrderCreation(TestCase):
    ...
    def test_uses_default_order_status_from_settings(self):
        add_product(self.basket, D('12.00'))
        with override_settings(IZI_INITIAL_ORDER_STATUS='A'):
            place_order(self.creator, basket=self.basket, order_number='1234')
        order = Order.objects.get(number='1234')
