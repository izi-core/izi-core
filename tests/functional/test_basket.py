...
from izi.apps.basket import reports
...
from izi.test.basket import add_product
...
class AnonAddToBasketViewTests(WebTestCase):
    ...
    def test_cookie_is_created(self):
        self.assertTrue('izi_open_basket' in self.response.test_app.cookies)

    def test_price_is_recorded(self):
        izi_open_basket_cookie = _unquote(self.response.test_app.cookies['izi_open_basket'])
        basket_id = izi_open_basket_cookie.split(':')[0]
...
class BasketThresholdTest(WebTestCase):
    ...
    def setUp(self):
        self._old_threshold = settings.IZI_MAX_BASKET_QUANTITY_THRESHOLD
        settings.IZI_MAX_BASKET_QUANTITY_THRESHOLD = 3

    def tearDown(self):
        settings.IZI_MAX_BASKET_QUANTITY_THRESHOLD = self._old_threshold
